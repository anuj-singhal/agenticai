...
class MCPAgent:
    ...
    #             session = async with stdio_client(server_params)
    ...
    async def initialize_servers(self):
        ...
        for config in self.server_configs:
            try:
                ...
                logger.info(f"Attempting to connect to server: {config['name']}")
                
                async with stdio_client(server_params) as session:
                    logger.info(f"Session created for: {config['name']}")
                    
                    try:
                        await session.initialize()
                        logger.info(f"Session initialized for: {config['name']}")
                    except Exception as init_error:
                        logger.error(f"Failed to initialize session for {config['name']}: {init_error}")
                        raise
                    ...
                    try:
                        ...
                        logger.error(f"Failed to discover capabilities for {config['name']}: {discover_error}")
                        # Don't raise here if capabilities discovery is optional
    ...
    async def _use_tool(self, server_name: str, tool_name: str, arguments: Dict[str, Any]) -> Any:
...
async def main():
    ...
    try:
        ...
        # Execute a task
        task = "List all Python files in the current directory and summarize their purposes"
